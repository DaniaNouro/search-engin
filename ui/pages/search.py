...
#
# # 1. Session state for query persistence across reruns
# if "query_val" not in st.session_state:
#     st.session_state.query_val = ""
#
# # 2. Primary query input (synced with session_state)
# query = st.text_input(
#     "Enter your query",
...
# )
#
# if st.session_state.query_input != st.session_state.query_val:
#     st.session_state.query_val = st.session_state.query_input
#
# current_query = st.session_state.query_val
# USER_ID = "default_user"
#
# # ── Interactive Query Assistance (Autocomplete + Spelling) ─────────────────
...
#             st.session_state.query_val = corrected_query
#             st.rerun()
#
#     # Context-Aware Query Autocomplete (Implicit Feedback from search_history)
...
#             if st.button(
#                 f"🔍 {comp_query}",
#                 key=f"comp_{idx}_{comp_query[:32]}",
#                 use_container_width=True,
#             ):
#                 st.session_state.query_val = comp_query
#                 st.rerun()
#
...
# )
#
# # 5. Dynamic hyperparameters
# with st.expander("🛠️ Advanced Model Configurations (Hyperparameters)", expanded=True):
#     k1, b = 1.5, 0.75
#     alpha, beta = 0.5, 0.5
...
#         with col1:
#             k1 = st.slider("Term Frequency Saturation (k1)", 1.2, 2.0, 1.5, step=0.1)
#         with col2:
#             b = st.slider("Document Length Normalization (b)", 0.5, 1.0, 0.75, step=0.05)
#
#     elif model == "Hybrid":
...
#         col1, col2 = st.columns(2)
#         with col1:
#             alpha = st.slider("Lexical Weight (α - BM25/TF-IDF)", 0.0, 1.0, 0.5, step=0.1)
#         with col2:
#             beta = st.slider("Semantic Weight (β - BERT)", 0.0, 1.0, 1.0 - alpha, step=0.1)
#
#             if alpha + beta != 1.0:
#                 st.caption(
...
#
# # ── Search Execution (Implicit Query Enrichment + Backend Retrieval) ─────────
# if st.button("Search"):
#     if not current_query.strip():
#         st.warning("Please enter a valid query.")
#     else:
#         # Orthographic refinement before enrichment and retrieval
#         refined_query = refine_query(current_query.strip())
#
#         # Implicit Query Enrichment & Re-weighting from search_history profile
#         formulation = formulate_query_with_context(refined_query, user_id=USER_ID)
...
#             )
#
#         # Persist the user's explicit query (not the enriched retrieval payload)
#         save_query(user_id=USER_ID, query_text=formulation.display_query, model_used=model)
#
